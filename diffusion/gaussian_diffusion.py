...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, raw_x=None, mask=None):
        """
        ...
        if not self.training and mask is not None:
            # print('raw_x.shape', raw_x.shape) # torch.Size([2, 4, 12, 32, 32]) / torch.Size([2, 5, 4, 32, 32])
            # print('mask.shape', mask.shape) # torch.Size([4, 1, 12, 32, 32]) / torch.Size([2, 5, 32, 32])
            # print('x.shape', x.shape) # torch.Size([2, 12, 4, 32, 32]) / torch.Size([2, 5, 4, 32, 32])
    ...
    def _vb_terms_bpd(
        ...
        out = self.p_mean_variance(
            model, x_t, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs, raw_x=x_start, mask=mask
        )
        kl = normal_kl(
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, mask=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            # mask, C B T H W
            # print('x_start.shape', x_start.shape) torch.Size([2, 4, 12, 32, 32]) - 4, 2, 12, 32, 32
            # print('mask.shape', mask.shape) torch.Size([1, 12, 4, 32, 32]) - 4, 1, 12, 32, 32
            # print('x_t.shape', x_t.shape) torch.Size([2, 12, 4, 32, 32]) - 4, 2, 12, 32, 32
            mask = mask.permute(2, 0, 1, 3, 4)
            model_input = x_start.permute(1, 0, 2, 3, 4) * (1-mask) + x_t.permute(2, 0, 1, 3, 4) * mask
            # C B T H W -> B T C H W
            model_input = model_input.permute(1, 2, 0, 3, 4) # 2, 12, 4, 32, 32
            model_output = model(model_input, t, **model_kwargs)
